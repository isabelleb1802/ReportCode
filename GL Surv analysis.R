...
surv <- surv %>%
  ...
  # Recode Infection1 to match fecundity analysis
  mutate(
...
# Remove rows with missing Lifespan (truly unrecoverable)
surv_clean <- surv %>%
...
# Check what's in the Control second infection subset
...
# Build surv_clean in one single pipe - do not run this more than once
surv_clean <- surv_raw %>%
  select(Vial_ID, Cohort, Genotype, Infection1, Infection2,
         Lifespan, Censor, Notes) %>%
  filter(!is.na(Lifespan)) %>%
...
# Also run Phag vs WT focused model
...
# Focus on the significant/interesting genotypes
...
# Also a comprehensive KM for all genotypes x re-infection
...
# Figure B — 3-Tap first infection only
...
# Figure C — DTI first infection only
...
# Advice 3 — Phag vs WT priming comparison
