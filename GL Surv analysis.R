...
# Creating 3 separate datasets for the infection treatments and combining
surv_unprimed <- surv_clean %>%
  filter(Genotype %in% c("WT", "Phag"),
         Infection1 == "Control",
         as.character(Infection2) == "DTI") %>%
  mutate(Priming = "Unprimed")

surv_3tap <- surv_clean %>%
  filter(Genotype %in% c("WT", "Phag"),
         Infection1 == "3 Tap",
         as.character(Infection2) == "DTI") %>%
  mutate(Priming = "3-Tap primed")

surv_dti <- surv_clean %>%
  filter(Genotype %in% c("WT", "Phag"),
         Infection1 == "DTI",
         as.character(Infection2) == "DTI") %>%
  mutate(Priming = "DTI primed")

# Combining all three
surv_priming_3panel <- bind_rows(surv_unprimed, surv_3tap, surv_dti) %>%
  mutate(
    Genotype = factor(Genotype,
                      levels = c("WT", "Phag"),
                      labels = c("Wildtype (WT)", "ΔPhag")),
    Priming = factor(Priming,
                     levels = c("Unprimed", "3-Tap primed", "DTI primed"))
  )


fig_priming <- ggsurvplot_facet(
  survfit(Surv(Lifespan, status) ~ Priming,
          data = surv_priming_3panel),
  data             = surv_priming_3panel,
  facet.by         = "Genotype",
  censor           = FALSE,
  pval             = TRUE,
  conf.int         = FALSE,
  palette          = c("#999999", "#E63946", "#1D3557"),
  legend.labs      = c("Unprimed", "3-Tap primed", "DTI primed"),
...
print(fig_priming)
ggsave("Figure_priming_3panel.png", fig_priming,
